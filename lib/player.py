...
class Player:
    ...
    def move(self, pressed_keys, upped_keys, platform_rects):
        if pg.K_d in pressed_keys and pg.Rect.collidelist(self.rect.move(PLAYER_SPEED, 0), platform_rects) == -1:
            self.player_position[0] += PLAYER_SPEED
        if pg.K_a in pressed_keys and pg.Rect.collidelist(self.rect.move(-PLAYER_SPEED, 0), platform_rects) == -1:
            self.player_position[0] -= PLAYER_SPEED
