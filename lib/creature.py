...
from lib.constants import PLAYER_FILENAME, SCALE, START_POSITION_X, PLAYER_SPEED, \
    ...
    SCREEN_HEIGHT, PLAYER_HEIGHT, JUMP_COUNT, TIME_OF_ATTACK
...
class Creature:
    def __init__(self, image, attack_image, position, health_points, damage):
        ...
        self.in_jump = False
        self.jumps_count = JUMP_COUNT

        self.not_idle_counter = 0 # Счётчик состояния покоя
        self.is_alive = True
    ...
    def move_physically(self, platform_rects):
        ...
        if self.walking_right and not self.walking_left:
            ...
            self.speed[0] = 0
        if self.walking_right and pg.Rect.collidelist(self.rect.move(PLAYER_SPEED * DELTA_T, 0), platform_rects) != -1:
            self.speed[0] = 0

        if self.walking_left and not self.walking_right:
            self.speed[0] = -PLAYER_SPEED
            self.looking_left = True
        if self.walking_left and pg.Rect.collidelist(self.rect.move(PLAYER_SPEED * -DELTA_T, 0), platform_rects) != -1:
